...
def convert_coco_to_yolo(coco_annotation_file):
    """
    Convert COCO format annotation file to YOLO pose format.
    """
        ...
        try:
            ...
            for kp_name, coords in keypoint_coords.items():
                ...
                if yolo_idx is not None and 'x' in coords and 'y' in coords:
                    yolo_keypoints[yolo_idx * 2] = str(coords['x'])
                    yolo_keypoints[yolo_idx * 2 + 1] = str(coords['y'])
            
            converted_line = ','.join(yolo_keypoints)
...
def create_cleaned_dataset(matches, output_folder, limit_of_videos=None, mode="override"):
    ...
    for img_num, (video_file, annotation_file) in enumerate(matches, 1):
        ...
        try:
            ...
            frame_annotations = convert_coco_to_yolo(annotation_file)
...
def main():
    ...
    output_folder = Path(args.video_folder).parent / f"cleaned_{video_folder_name}"
